...
public class GoodsServiceImpl implements IGoodsService {
    ...
    public Boolean deductGoodsInventory(List<DeductGoodsInventory> deductGoodsInventories) {
        ...
        List<Long> collect = deductGoodsInventories.stream()
                .map(DeductGoodsInventory::getGoodsId)
                .collect(Collectors.toList());

        //通过商品ids查询商品

        List<EcommerceGoods> ecommerceGoods = null;

        try {
            ecommerceGoods = IterableUtils.toList(
                    ecommerceGoodsDao.findAllById(collect)
            );
        } catch (Exception e) {
            log.info(e.toString());
